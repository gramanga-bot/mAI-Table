            ...
            <div className="bg-gray-800 rounded-2xl shadow-2xl shadow-black/50 border border-gray-700 w-full max-w-2xl animate-fade-in-up overflow-hidden">
                <div className="p-8">
                    <div className="text-center mb-6">
                        <Icon name="bell" className="w-16 h-16 bg-green-500/20 text-green-400 rounded-full p-3 mx-auto mb-4" />
                        <h2 className="text-3xl font-bold text-white">Notifica Inviata!</h2>
                        <p className="text-gray-400">Il cliente è stato avvisato della conferma della sua prenotazione.</p>
                    </div>

                     <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700 mb-6 text-center">
                        <p className="text-lg text-white">
                            Conferma per <strong className="font-semibold text-amber-400">{details.name}</strong>, per <strong className="font-semibold text-amber-400">{totalGuests}</strong> persone il <strong className="font-semibold text-amber-400">{new Date(details.date).toLocaleDateString('it-IT', { month: 'long', day: 'numeric' })}</strong> alle <strong className="font-semibold text-amber-400">{details.time}</strong>.
                        </p>
                    </div>

                    <p className="text-center text-gray-500 text-sm mb-4">È stato inviato il seguente messaggio di conferma:</p>
                ...
                <div className="bg-gray-800/50 p-4 border-t border-gray-700">
                     <button onClick={onClose} className="w-full bg-amber-500 text-gray-900 font-bold py-3 px-4 rounded-lg hover:bg-amber-400 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-amber-500 text-lg">
                        Fatto
                    </button>
                </div>
            </div>
